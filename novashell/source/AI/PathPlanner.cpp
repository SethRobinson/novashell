...
double PathPlanner::GetCostToNode(unsigned int NodeIdx)const
{
  ...
  double cost =Vec2DDistance(m_pOwner->GetPos(),
...
double PathPlanner::GetCostToClosestItem(unsigned int GiverType)const
{
  ...
  //type GiverType
  const Map::TriggerSystem::TriggerList& triggers = m_pOwner->GetWorld()->GetMap()->GetTriggers();

  Map::TriggerSystem::TriggerList::const_iterator it;
...
void PathPlanner::SmoothPathEdgesUltraQuick(Path& path)
{
}
...
bool PathPlanner::IsPathNeededToGetToTarget(CL_Vector2 &TargetPos)
{

	float dist = Vec2DDistance(m_pOwner->GetPos(), TargetPos);
	...
	if (dist < m_pWorld->GetNavGraph()->GetNodeMaxLinkDistance())
	{
		return !m_pOwner->CanWalkTo(TargetPos, false);
	}
	return true;
}
...
{ 
  ...
  #ifdef SHOW_NAVINFO
     LogMsg("Closest node to bot is %d", ClosestNodeToBot);
#endif
...
if (GetGameLogic()->GetShowPathfinding())
  ...
  typedef Graph_SearchAStar_TS<NavGraphManager::NavGraph, Heuristic_Euclid> AStar;
  ...
  //now, we're ready for CycleOnce() to be called until the path is completely computed
  return true;
}
...
{    
  ...
  typedef FindActiveTrigger<Trigger<MovingEntity> > t_con; 
...
//----------------------------------------------------------------------------- 
CL_Vector2 PathPlanner::GetNodePosition(int idx)const
{
