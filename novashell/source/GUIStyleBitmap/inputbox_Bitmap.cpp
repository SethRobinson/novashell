...
CL_InputBox_Bitmap::CL_InputBox_Bitmap(
    ...
    first_paint(false)
{
	...
	slots.connect(inputbox->sig_paint(), this, &CL_InputBox_Bitmap::on_paint);
}
...
void CL_InputBox_Bitmap::on_paint()
{
    on_paint_default();
}

void CL_InputBox_Bitmap::on_changed()
{
    // TODO: 
}

void CL_InputBox_Bitmap::on_paint_default()
{
    // Fill
    //-------
    CL_Color const clr_background(0, 40, 0, 60);
    CL_Rect const rc_main(inputbox->get_screen_rect());
	CL_StyleManager_Bitmap::fill_rounded_rect(rc_main, clr_background);

    // Outline
    CL_Color const clr_outline(175, 205, 148);
	CL_StyleManager_Bitmap::draw_rounded_rect(rc_main, clr_outline);

	on_paint_input();
}

void CL_InputBox_Bitmap::on_paint_input()
{
