...
CL_CheckBox_Bitmap::CL_CheckBox_Bitmap(
	...
	checkbox(checkbox)
{
	...
	slot_paint = checkbox->sig_paint().connect_virtual(
		this, &CL_CheckBox_Bitmap::on_paint);
	slot_get_preferred_size = checkbox->sig_get_preferred_size().connect(
		this, &CL_CheckBox_Bitmap::on_get_preferred_size);
}
...
void CL_CheckBox_Bitmap::on_get_preferred_size(CL_Size &size)
{
	size.width = font->get_width(checkbox->get_text()) + 28;
	size.height = 35;
}

void CL_CheckBox_Bitmap::on_paint(CL_SlotParent_v0 &super)
{
