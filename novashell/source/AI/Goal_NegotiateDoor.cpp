...
//-----------------------------------------------------------------------------
Goal_NegotiateDoor::
Goal_NegotiateDoor(MovingEntity*   pBot,
                   PathEdge     edge,
                   bool         LastEdge):Goal_Composite<MovingEntity>(pBot, goal_negotiate_door),
                                        m_PathEdge(edge),
...
void Goal_NegotiateDoor::Activate()
{
  ...
  CL_Vector2 posSw = m_pOwner->GetWorld()->GetPosOfClosestSwitch(m_pOwner->Pos(),
                                                          m_PathEdge.DoorID());
