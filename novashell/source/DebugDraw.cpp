...
void DebugDraw::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
	...
	for (int32 i = 0; i < vertexCount; ++i)
	{
		...
		a = FromPhysicsSpace(vertices[i]);
		
		if (i+1 == vertexCount)
		{
			b = FromPhysicsSpace(vertices[0]);
...
/*
	CL_Vector2 p1 = FromPhysicsSpace(xf.position), p2;
	
	//LogMsg("p1 is %s", PrintVector(p1).c_str());
	const float32 k_axisScale = 0.4f;
	glBegin(GL_LINES);

	glColor3f(1.0f, 0.0f, 0.0f);
	glVertex2f(p1.x, p1.y);
	p2 = p1 +  *(CL_Vector2*)&(k_axisScale * xf.R.col1);
	glVertex2f(p2.x, p2.y);

	glColor3f(0.0f, 1.0f, 0.0f);
	glVertex2f(p1.x, p1.y);
	p2 = p1 + *(CL_Vector2*)&(k_axisScale * xf.R.col2);
	glVertex2f(p2.x, p2.y);
