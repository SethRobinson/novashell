#include <API/core.h> // for CL_DomElement
...
CL_ListBox_Bitmap::CL_ListBox_Bitmap(
	CL_ListBox * listbox,
    ...
    listbox_type_(type_normal)
{
    ...
    {
        CL_ScrollBar_Bitmap * sb_type = (CL_ScrollBar_Bitmap*)scrollbar_->get_style();
        if (sb_type)
            sb_type->set_type(CL_ScrollBar_Bitmap::type_listbox);
    }
	...
	slots_.connect(listbox_->sig_set_options(), this, &CL_ListBox_Bitmap::on_set_options);
	slots_.connect(listbox_->sig_paint(), this, &CL_ListBox_Bitmap::on_paint_background);
	slots_.connect(client_area->sig_mouse_down(), this, &CL_ListBox_Bitmap::on_mouse_down);
	slots_.connect(client_area->sig_paint(), this, &CL_ListBox_Bitmap::on_paint_listbox);
	slots_.connect(listbox_->sig_resize(), this, &CL_ListBox_Bitmap::on_resize);
	slots_.connect(listbox_->sig_item_added(), this, &CL_ListBox_Bitmap::on_item_added);
	slots_.connect(listbox_->sig_item_removed(), this, &CL_ListBox_Bitmap::on_item_removed);
	slots_.connect(listbox_->sig_clear(), this, &CL_ListBox_Bitmap::on_clear);
	slots_.connect(scrollbar_->sig_value_changed(), this, &CL_ListBox_Bitmap::on_scroll_change);
	slots_.connect(listbox_->sig_get_preferred_size(), this, &CL_ListBox_Bitmap::on_get_preferred_size);
...
void CL_ListBox_Bitmap::on_get_preferred_size(CL_Size &size)
{
	...
	for (it = items.begin(); it != items.end(); ++it)
	{
		...
		tempWidth = font_->get_width((*it)->str);
	...
	if (maxWidth == 0)
	{
		//not really ready to do this yet
		maxWidth = 100;
	}

	size.width = maxWidth+28;
	size.height = (font_->get_height() * items.size()) + 12;
}

void CL_ListBox_Bitmap::on_paint_background()
{
 
	if (listbox_->get_count() == 0) return;
    ...
    // main
    CL_Color const clr_background = style_->GetColor(CL_StyleManager_Bitmap::COLOR_LISTBOX_BG);
    CL_Rect rc_main = listbox_->get_screen_rect();
    style_->fill_rounded_rect(rc_main, clr_background);

    // Outline
    CL_Color clr_outline = style_->GetColor(CL_StyleManager_Bitmap::COLOR_LISTBOX_BORDER);
    CL_Rect rc_outline = rc_main;
    style_->draw_rounded_rect(rc_outline, clr_outline);
}

void CL_ListBox_Bitmap::on_paint_listbox()
{
...
//        font_->set_color(152.0f / 255, 211.0f / 255, 101.0f / 255);
		font_->set_color(style_->GetColor(CL_StyleManager_Bitmap::COLOR_LISTBOX_UNSELECTED));
		...
		for (it = items.begin(); it != items.end(); ++it, ++pos)
        {
            ...
            if (pos == listbox_->get_current_item())
            {
                CL_Display::fill_rect(
                   ...
                   style_->GetColor(CL_StyleManager_Bitmap::COLOR_LISTBOX_SELECTION_BG));
				font_->set_color(style_->GetColor(CL_StyleManager_Bitmap::COLOR_LISTBOX_SELECTED));
    ...
    if (listbox_type_ == type_levels_list)
    {
        ...
        for (it = items.begin(); it != items.end(); ++it, ++pos)
        {
			...
			CL_ListBoxItem_Bitmap * item = (CL_ListBoxItem_Bitmap *)(*it);
			std::string text;
			CL_Color text_color;
			if (item->get_solved())
			{
...
void CL_ListBox_Bitmap::on_item_added(int index)
{
	update_scrollbar(true);
	listbox_->find_preferred_size();
}

void CL_ListBox_Bitmap::on_item_removed(int index)
{
	update_scrollbar(true);
	listbox_->find_preferred_size();
}

void CL_ListBox_Bitmap::on_clear()
{
	update_scrollbar(true);
}

void CL_ListBox_Bitmap::on_set_options(const CL_DomElement &options)
{
...
void CL_ListBox_Bitmap::on_scroll_change(int new_offset)
{
	listbox_->set_top_item(new_offset);
}

void CL_ListBox_Bitmap::on_mouse_down(const CL_InputEvent &key)
{
...
void CL_ListBox_Bitmap::on_resize(int old_width, int old_height)
{
...
void CL_ListBox_Bitmap::update_scrollbar(bool resize)
{	
