...
public:
  ...
  void SetName(const string &name) {m_goalName = name;}

  virtual const char * GetName() const {return m_goalName.c_str();}

  virtual void RenderAtPos(CL_Vector2& pos)const;
...
{ 
  ...
  {    
    ...
    assert(m_SubGoals.front() != NULL && "Uh, this is bad");
...
//-----------------------------------------------------------------------------
template <class entity_type>
bool Goal_Composite<entity_type>::ForwardMessageToFrontMostSubgoal(const Message& msg)
{
  if (!m_SubGoals.empty())
  {
...
//-------------------------- RenderAtPos --------------------------------------
template <class entity_type>
void  Goal_Composite<entity_type>::RenderAtPos(CL_Vector2& pos)const
{
  Goal<entity_type>::RenderAtPos(pos);
...
template <class entity_type>
void  Goal_Composite<entity_type>::LostFocus()
{
