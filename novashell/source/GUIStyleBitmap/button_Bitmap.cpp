...
CL_Button_Bitmap::CL_Button_Bitmap(
	CL_Button *button,
    ...
    resources_(style->get_resources())
{
    ...
    CL_Size text_size = font_->get_size(button_->get_text().c_str());
	{
		slots_.connect(button_->sig_mouse_enter(),
			this, &CL_Button_Bitmap::on_mouse_enter);
		slots_.connect(button_->sig_mouse_leave(),
			this, &CL_Button_Bitmap::on_mouse_leave);
		slots_.connect(timer_.sig_timer(),
			this, &CL_Button_Bitmap::on_timer);
	    slots_.connect(button_->sig_get_preferred_size(),
		    this, &CL_Button_Bitmap::on_get_preferred_size);
    }
    ...
    slots_.connect(button_->sig_paint(), this, &CL_Button_Bitmap::on_paint);
	slots_.connect(button_->sig_pressed(), this, &CL_Button_Bitmap::on_pressed);
	slots_.connect(button_->sig_released(), this, &CL_Button_Bitmap::on_released);
	slots_.connect(button_->sig_set_options(), this, &CL_Button_Bitmap::on_set_options);
...
void CL_Button_Bitmap::on_get_preferred_size(CL_Size &size)
{
...
void CL_Button_Bitmap::on_mouse_enter()
{
	timer_.enable();
	highlighted_ = true;
...
void CL_Button_Bitmap::on_mouse_leave()
{
	timer_.enable();
	highlighted_ = false;
...
void CL_Button_Bitmap::on_timer()
{
...
void CL_Button_Bitmap::on_paint()
{
	if(button_type_ == type_text)
        ...
        if (button_type_ == type_bitmap)
            ...
            cl_throw_error("CL_Button_Bitmap::on_paint, Unknown button type " +
                CL_String::from_int(button_type_));
}

void CL_Button_Bitmap::on_paint_texture()
{
...
void CL_Button_Bitmap::on_paint_anim()
{
...
void CL_Button_Bitmap::on_pressed()
{
	down_ = true;
}

void CL_Button_Bitmap::on_released()
{
	down_ = false;
}

void CL_Button_Bitmap::set_button_type(ButtonType const button_type)
{
    button_type_ = button_type;
	button_->find_preferred_size();
}

void CL_Button_Bitmap::set_button_size(ButtonSize const button_size)
{
    if (button_type_ == type_text)
    {
...
void CL_Button_Bitmap::set_surface_up(CL_Surface * surface_up)
{
    delete surface_up_;
...
void CL_Button_Bitmap::set_surface_down(CL_Surface * surface_down)
{
    delete surface_down_;
    surface_down_ = surface_down;
}

void CL_Button_Bitmap::set_surface_highlighted(CL_Surface * surface_highlighted)
{
    delete surface_highlighted_;
    surface_highlighted_ = surface_highlighted;
}

void CL_Button_Bitmap::set_surface_disabled(CL_Surface * surface_disabled)
{
    delete surface_disabled_;
    surface_disabled_ = surface_disabled;
}

void CL_Button_Bitmap::set_surfaces(CL_Surface * surface_up,
...
void CL_Button_Bitmap::set_font(CL_Font * font)
{
    if (font)
    {
        delete font_;
...
void CL_Button_Bitmap::on_set_options(const CL_DomElement &options)
{
