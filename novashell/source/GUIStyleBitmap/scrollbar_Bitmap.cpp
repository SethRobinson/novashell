...
CL_ScrollBar_Bitmap::CL_ScrollBar_Bitmap(
    ...
    scrollbar_type_(scrollbar_type)
{
    ...
    else
    {
        ...
        CL_Button_Bitmap * btn_style = (CL_Button_Bitmap*)button_increase_->get_style();
        if (btn_style)
        {
            btn_style->set_surfaces(
                ...
                new CL_Surface("ListBox/scroll_btn_down", resources_),
                new CL_Surface("ListBox/scroll_btn_down", resources_));
            btn_style->set_button_type(CL_Button_Bitmap::type_bitmap);
        }
        button_decrease_ = new CL_Button(scrollbar_, style_);
        btn_style = (CL_Button_Bitmap*)button_decrease_->get_style();
        if (btn_style)
        {
            btn_style->set_surfaces(
	...
	slots_.connect(client_area->sig_paint(), this, &CL_ScrollBar_Bitmap::on_paint);
	slots_.connect(button_increase_->sig_pressed(), this, &CL_ScrollBar_Bitmap::start_scroll, 1);
	slots_.connect(button_decrease_->sig_pressed(), this, &CL_ScrollBar_Bitmap::start_scroll, -1);
	slots_.connect(button_increase_->sig_released(), this, &CL_ScrollBar_Bitmap::stop_scroll);
	slots_.connect(button_decrease_->sig_released(),	this, &CL_ScrollBar_Bitmap::stop_scroll);
	slots_.connect(scrollbar_->sig_get_preferred_size(), this, &CL_ScrollBar_Bitmap::on_get_preferred_size);
    slots_.connect(timer_scroll_.sig_timer(), this, &CL_ScrollBar_Bitmap::on_timer_scroll);
}
...
void CL_ScrollBar_Bitmap::start_scroll(int delta)
{
	scroll_delta_ = delta;
...
void CL_ScrollBar_Bitmap::stop_scroll()
{
	timer_scroll_.disable();
}

void CL_ScrollBar_Bitmap::on_timer_scroll()
{
	scrollbar_->set_value(scrollbar_->get_value() + scroll_delta_);
}

void CL_ScrollBar_Bitmap::on_paint()
{
    if (!initialized_)
    {
        CL_Rect rect_decrease;
...
void CL_ScrollBar_Bitmap::on_get_preferred_size(CL_Size &size)
{
}

void CL_ScrollBar_Bitmap::set_type(ScrollBarType scrollbar_type)
{
        ...
        if (scrollbar_type == type_listbox)
        {
            ...
            if (scrollbar_type_ == type_normal)
            {
                ...
                if (scrollbar_type_ == type_normal)
                {
                    ...
                    CL_Button_Bitmap * btn_style = (CL_Button_Bitmap*)button_decrease_->get_style();
                    if (btn_style)
                        btn_style->set_button_type(CL_Button_Bitmap::type_text);
                    btn_style = (CL_Button_Bitmap*)button_increase_->get_style();
                    if (btn_style)
                        btn_style->set_button_type(CL_Button_Bitmap::type_text);
                }
            }
